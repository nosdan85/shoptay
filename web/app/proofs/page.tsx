...
export default function ProofsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-900">
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {proofs.map((proof, idx) => (
                <div key={proof.id} className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-5 border border-slate-700/50 hover:border-slate-600/50 transition-all animate-vouch-entrance" style={{ animationDelay: `${idx * 80}ms` }}>
                  {/* Images Grid */}
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      {proof.imageUrls.map((url, idx) => (
                        <img
                          ...
                          onClick={() => { setClosing(false); setLightboxImage(url); }}
                        />
                      ))}
                    </div>
                  ) : (
      ...
      {/* Lightbox Modal */}
      {lightboxImage && (
        <div className={"fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4 " + (closing ? "animate-fade-out" : "animate-fade-in")}
        ...
        >
          ...
          <img
            src={lightboxImage}
            alt="Full size proof"
            className={"max-w-full max-h-full object-contain rounded-lg " + (closing ? "animate-modal-zoom-out" : "animate-modal-zoom-in")}
