            ...
            <div className="rounded-[18px] border border-[#1E1E1E] bg-[#111111] p-6 space-y-4 animate-section-enter">
              ...
              {step === "delivery" && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">{slots.length === 0 && <p className="text-sm text-[#B5B5B5]">No available slots.</p>}
                    {slots.map((s) => (
                      <button key={s.id} onClick={() => setPickedSlot(s.id)} className={"w-full rounded-[16px] border p-4 text-left transition-all " + (pickedSlot === s.id ? "border-[#2F9BE6] bg-[#49B6FF]/10" : "border-[#1E1E1E] bg-[#050505] hover:border-[#1E1E1E]")}>
                        <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-[#2F9BE6]" /><span className="text-sm font-medium">{s.customerStartText} - {s.customerEndText}</span></div>
                      </button>
                    ))}
                  </div>
